Engine.Plugins.users = {
	...
	init:function(){
		Engine.GUI.Sidebar.Nav.add('Users', 'administration');
	},
	load:{
		index:function(){
			Engine.Builder.card($('#pagecontent'),{ title: 'Users', icon: 'users'}, function(card){
				Engine.request('users','read',{
					data:{options:{ link_to:'UsersIndex',plugin:'users',view:'index' }},
				},function(result) {
					var dataset = JSON.parse(result);
					if(dataset.success != undefined){
						for(const [key, value] of Object.entries(dataset.output.dom)){ Engine.Helper.set(Engine.Contents,['data','dom','users',value.username],value); }
						for(const [key, value] of Object.entries(dataset.output.raw)){ Engine.Helper.set(Engine.Contents,['data','raw','users',value.id],value); }
						Engine.Builder.table(card.children('.card-body'), dataset.output.dom, {
							...
							Engine.Plugins.users.element.table.index = response.table;
		...
		details:function(){
			...
			Engine.request(url.searchParams.get("p"),'get',{data:{id:id,key:'id'}},function(result){
				...
				if(dataset.success != undefined){
					...
					Engine.GUI.Layouts.details.build(dataset.output,container,{title:"User Details",image:bgImage},function(data,layout){
						if(layout.main.parents().eq(2).parent('.modal-body').length > 0){
							var modal = layout.main.parents().eq(2).parent('.modal-body').parents().eq(2);
							if(Engine.Auth.validate('plugin', 'users', 3)){
						...
						Engine.GUI.Layouts.details.tab(data,layout,{icon:"fas fa-history",text:Engine.Contents.Language["History"]},function(data,layout,tab,content){
							Engine.Helper.set(Engine.Contents,['layouts','users',data.this.raw.id,layout.main.attr('id')],layout);
							...
							// Clear
							if(Engine.Auth.validate('custom', 'users_clear', 1)){
								Engine.GUI.Layouts.details.control(data,layout,{color:"danger",icon:"fas fa-snowplow",text:Engine.Contents.Language["Clear"]},function(data,layout,button){
									button.off().click(function(){
										Engine.request('users','clear',{ data:data.this.raw },function(){
											Engine.Plugins.users.load.details();
										});
									});
								});
							}
							...
							Engine.GUI.Layouts.details.data(data,layout,options);
							// Phone
							if(Engine.Auth.validate('custom', 'users_phone', 1)){
								options.field = "phone";
								options.td = '';
								options.td += '<td>';
							...
							Engine.GUI.Layouts.details.data(data,layout,options,function(data,layout,tr){});
							// Notes
							if(Engine.Helper.isSet(Engine.Plugins,['notes']) && Engine.Auth.validate('custom', 'users_notes', 1)){
								Engine.GUI.Layouts.details.tab(data,layout,{icon:"fas fa-sticky-note",text:Engine.Contents.Language["Notes"]},function(data,layout,tab,content){
									...
									if(Engine.Auth.validate('custom', 'users_notes', 2)){
								...
								Engine.Plugins.users.Events.notes(data,layout);
							}
							// Contacts
							if(Engine.Helper.isSet(Engine.Plugins,['contacts']) && Engine.Auth.validate('custom', 'users_contacts', 1)){
								Engine.GUI.Layouts.details.tab(data,layout,{icon:"fas fa-address-book",text:Engine.Contents.Language["Contacts"]},function(data,layout,tab,content){
									...
									if(Engine.Auth.validate('custom', 'users_contacts', 2)){
										var html = '';
									...
									if(Engine.Helper.isSet(data,['relations','contacts'])){
										for(var [id, relation] of Object.entries(data.relations.contacts)){
											if(relation.isActive||Engine.Auth.validate('custom', 'users_contacts_isActive', 1)){
												Engine.Plugins.users.GUI.contact(relation,layout);
											}
										}
									}
								});
								Engine.Plugins.users.Events.contacts(data,layout);
							}
							// Created
							...
							Engine.GUI.Layouts.details.button(data,layout,{icon:icon},function(data,layout,button){
								button.off().click(function(){
									if(button.find('i').hasClass( "fa-bell" )){
										...
										Engine.request("users",'subscribe',{data:{id:data.this.raw.id}},function(answer){
							...
							for(var [rid, relations] of Object.entries(data.relationships)){
								for(var [uid, relation] of Object.entries(relations)){
									if(Engine.Helper.isSet(Engine.Plugins,[relation.relationship]) && (Engine.Auth.validate('custom', 'users_'+relation.relationship, 1) || relation.owner == Engine.Contents.Auth.User.username) && Engine.Helper.isSet(data,['relations',relation.relationship,relation.link_to])){
										var details = {};
										...
										if(!Engine.Helper.isSet(details,['isActive'])||(Engine.Helper.isSet(details,['isActive']) && details.isActive)||(Engine.Helper.isSet(details,['isActive']) && !details.isActive && (Engine.Auth.validate('custom', 'users_'+relation.relationship+'_isActive', 1)||Engine.Auth.validate('custom', relation.relationship+'_isActive', 1)))){
											switch(relation.relationship){
												case"notes":
													Engine.Builder.Timeline.add.card(layout.timeline,details,'sticky-note','warning',function(item){
														...
														if(Engine.Auth.validate('custom', 'users_notes', 4)){
	...
	GUI:{
		...
		contact:function(dataset,layout,plugin = 'contacts'){
			...
			area.prepend(Engine.Plugins.users.GUI.card(dataset));
			var card = area.find('div.col-sm-12.col-md-6').first();
			if(Engine.Auth.validate('custom', 'users_'+plugin+'_btn_edit', 1)){
				card.find('div.btn-group').append(Engine.Plugins.users.GUI.button(dataset,{id:'id',color:'warning',icon:'fas fa-edit',action:'edit',content:Engine.Contents.Language['Edit']}));
			}
			if(Engine.Auth.validate('custom', 'users_'+plugin+'_btn_delete', 1)){
				card.find('div.btn-group').append(Engine.Plugins.users.GUI.button(dataset,{id:'id',color:'danger',icon:'fas fa-trash-alt',action:'delete',content:''}));
			}
		},
			...
			td.find('button').off().click(function(){
				...
				switch(button.attr('data-action')){
					case"details":
						...
						break;
					case"remove":
						Engine.request('users','unlink',{data:{id:dataset.this.raw.id,relationship:{relationship:defaults.key,link_to:button.attr('data-id')}}},function(result){
						...
						Engine.Builder.modal($('body'), {
							...
							footer.find('button[data-action="add"]').off().click(function(){
								if((typeof body.find('select').select2('val') !== "undefined")&&(body.find('select').select2('val') != '')){
									Engine.request('users','link',{data:{id:dataset.this.dom.id,relationship:{relationship:defaults.key,link_to:body.find('select').select2('val')}}},function(result){
										var sub_dataset = JSON.parse(result);
										if(sub_dataset.success != undefined){
											...
											var html = Engine.Plugins.users.GUI.buttons.details(sub_dataset.output.dom,{
											...
											Engine.Plugins.users.Events.users(dataset,layout,defaults);
										}
									});
									modal.modal('hide');
								} else {
			...
			if(Engine.Auth.validate('custom', 'users_notes', 2)){
				layout.content.notes.find('button').off().click(function(){
				  if(!layout.content.notes.find('textarea').summernote('isEmpty')){
				    ...
				    Engine.request('users','note',{data:note},function(result){
		...
		contacts:function(dataset,layout,options = {},callback = null){
			...
			if(Engine.Auth.validate('custom', 'users_contacts', 2)){
				contacts.find('.addContact').off().click(function(){
					Engine.CRUD.create.show({ plugin:'contacts', keys:skeleton, set:{isActive:'true',relationship:'users',link_to:dataset.this.raw.id} },function(created,user){
						if(created){
							user.dom.name = '';
							...
							Engine.Plugins.users.GUI.contact(user.dom,layout);
							Engine.Plugins.users.Events.contacts(dataset,layout);
				...
				switch($(this).attr('data-action')){
					...
					case"delete":
						...
						Engine.CRUD.delete.show({ keys:contact,key:'name', modal:true, plugin:'contacts' },function(user){
							if(contacts.find('[data-id="'+contact.id+'"]').find('.ribbon-wrapper').length > 0 || !Engine.Auth.validate('custom', 'users_contacts_isActive', 1)){
								contacts.find('[data-id="'+contact.id+'"]').remove();
								layout.timeline.find('[data-type="address-card"][data-id="'+contact.id+'"]').remove();
							}
							if(contact.isActive && Engine.Auth.validate('custom', 'users_contacts_isActive', 1)){
